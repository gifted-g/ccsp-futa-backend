...
import (
    ...
    appdb "ccsp-futa-alumni/db" // FIX: Renamed import to 'appdb' to avoid conflict
...
func CreateChannelHandler(c *gin.Context) {
    ...
    // FIX: Use appdb.DB
    if err := appdb.DB.Create(&ch).Error; err != nil {
    ...
    for _, m := range req.Members {
        ...
        // FIX: Use appdb.DB
        appdb.DB.Create(&member)
    }
    
    // add creator as member if not present
    // FIX: Use appdb.DB
    appdb.DB.FirstOrCreate(&models.ChatMember{}, models.ChatMember{ChannelID: ch.ID, UserID: uuid.MustParse(creator), Role: "admin", JoinedAt: time.Now()})
...
func GetChannelHandler(c *gin.Context) {
    ...
    // FIX: Use appdb.DB
    if err := appdb.DB.Preload("Members").Where("id = ?", cid).First(&ch).Error; err != nil {
...
func AddChannelMemberHandler(c *gin.Context) {
    ...
    // FIX: Use appdb.DB
    if err := appdb.DB.Create(&member).Error; err != nil {
...
func PostMessageHandler(c *gin.Context) {
    ...
    // FIX: Use appdb.DB
    if err := appdb.DB.Create(&msg).Error; err != nil {
...
func ListMessagesHandler(c *gin.Context) {
    ...
    // FIX: Use appdb.DB
    if err := appdb.DB.Where("channel_id = ?", cid).Order("created_at desc").Limit(limit).Find(&msgs).Error; err != nil {
